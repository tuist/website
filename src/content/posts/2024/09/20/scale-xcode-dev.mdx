---
title: "Shall we land the Xcode plane?"
categories: ["Releases"]
...
In this case, the convenience of implicitness manifests as implicitly linked dependencies and implicitly resolved linking types (static or dynamic) through [mergeable libraries](https://developer.apple.com/documentation/xcode/configuring-your-project-to-use-mergeable-libraries). These are additional variables that introduce build-time complexity and variability, which can not only impact build performance but also make certain features, like Swift Previews, work inconsistently.
...
While we may not fully understand how Apple engineers manage this reconciliation internally, we recognize the complexity involved. Still, we remain optimistic: imagine if the time currently spent resolving these integration challenges could be redirected toward enhancing features. It's an exciting prospect, and we believe the groundwork being laid today will lead to a more streamlined and robust future for both Xcode and SPM.
