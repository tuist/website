...
~3 minutes doesn't seem that bad, but the times get worse when you add more Swift Macros to your project.

## A non-API-stable SwiftSyntax

If you add multiple macros that depend on multiple SwiftSyntax versions, the different versions of SwiftSyntax will have to be built. Let's assume five Swift Macros with five different versions of SwiftSyntax. The total increase in build time will be roughly the sum of the time it takes to compile each SwiftSyntax tree. Taking the above number as a reference, that would amount to a total of ~15 minutes.
...
What this means is that even if they made SwiftSyntax ABI stable, providing binaries of the package wouldn't work in non-Darwin environments. Will Apple invest in that effort? That's a big question that only Apple can answer, but past work on non-Darwin platforms was traditionally done by the community.
...
If you want to see them in action, you can play with one of the Tuist project's fixtures:
...
<Image src={cachedSwiftMacro} alt="An screenshot that shows an Xcode project where the file navigator includes the one source file contained in the framework, and all the external dependencies cached as XCFrameworks" style="max-width: 40vw" />
