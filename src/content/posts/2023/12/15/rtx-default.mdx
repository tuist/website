---
title: "We are deprecating tuistenv in favor of rtx"
categories: ["rtx", "asdf"]
excerpt: "In this blog post, we share why we are deprecating tuistenv in favor of rtx, a runtime executor that allows you to manage multiple versions of a tool and activate the right one when you choose a directory in your terminal."
author: pepicrft
---
...
`rtx` can manage multiple versions of a tool,
...
`rtx` includes handy commands like `ls-remote`, which lists all the remotely-available versions:
...
After thorough usage and having a great experience, we've decided to deprecate `tuistenv` in favor of `rtx`.
If you are currently using `tuistenv` to manage Tuist versions, we recommend you to migrate to `rtx` by following the instructions below.
...
rtx install tuist@3.36.0
rtx install tuist@3
rtx use tuist@3.36.0
```
...
We belive `rtx` is the right solution for Tuist. It provides a great developer experience, it's very well maintained and stable, and most importantly, it'll allow us to focus on the core features that make Tuist great.